...
public class MyBigInteger  implements Comparable<MyBigInteger> {   //  We will cover what "implements Comparable" means later
	...
	public MyBigInteger dividedBy( MyBigInteger  divisor ) throws Exception{
		...
		if (this.base != divisor.base){
			throw new Exception("Bases must be the same in divideBy method");
		}

		MyBigInteger quotient = new MyBigInteger(divisor.base);
		int divisorSize = divisor.coefficients.size();
		boolean flag = true;
		int i = 0;
		MyBigInteger subdividee = new MyBigInteger(divisor.base);
		MyBigInteger subquotient;
		while(flag) {
			subdividee.coefficients.add(0,this.coefficients.get(this.coefficients.size() -1 -i));
			subdividee = subdividee.removeZero(subdividee);
			subquotient = subdividee.slowdividedBy(divisor);
			quotient.coefficients.add(0,subquotient.coefficients.get(0));
			subdividee = subdividee.minus(subquotient.times(divisor));
			i++;
			if(i == this.coefficients.size())
				flag = false;
		}
		quotient = quotient.removeZero(quotient);
	...
	public MyBigInteger convert(int newBase) throws Exception {
		...
		MyBigInteger modulus = new MyBigInteger(this.base);
    	//   ADD YOUR CODE BELOW HERE

		//Remove excess zeros
		remainder = remainder.removeZero(remainder);

		if (newBase == this.base)
			return this;
		if(newBase == 10){
			for (int i = 0; i < this.coefficients.size(); i++){
				result = result.plus(new MyBigInteger(remainder.coefficients.get(i), 10).times(power(this.base,i)));
			}
		}else{
			boolean flag = true;
			if(this.base == 10){
				while(flag){
					remainder = remainder.removeZero(remainder);
					modulus = remainder.mod(new MyBigInteger(newBase, 10));
					result.coefficients.add(modulus.coefficients.get(0));
					remainder = remainder.dividedBy(new MyBigInteger(newBase, 10));
					if(remainder.compareTo(new MyBigInteger(0,10)) == 0){
						flag = false;
					}
				}
			}else{
				remainder = remainder.removeZero(remainder);
				result = remainder.convert(10);
				result = result.removeZero(result);
				result = result.convert(newBase);
			}
		}
		//   ADD YOUR CODE ABOVE HERE
	...
	public MyBigInteger power(int base, int index) throws Exception{
		MyBigInteger power = new MyBigInteger(1,10);
		MyBigInteger bases = new MyBigInteger(base,10);
		if (index == 0){
			return power;
		}
		for(int i=0; i < index; i++ ){
			power = power.times(bases);
		}
		return power;
	}
	...
	public ArrayList<MyBigInteger>  primeFactors() throws Exception {
		...
		int sqrt = 2;
		int square_val = 1;
		while(this.compareTo(new MyBigInteger(square_val, this.base)) != -1){
			if (sqrt == 45000)
				break;
			sqrt = sqrt + 1;
			square_val = sqrt*sqrt;
		}

		ArrayList<MyBigInteger> primes = primesToN(sqrt, this.base);

		for (int i = 0; i < primes.size(); i++){
			while (number.mod(primes.get(i)).coefficients.size() == 1 &&  number.mod(primes.get(i)).coefficients.get(0) == 0 ){
				number = number.dividedBy(primes.get(i));
	...
	public MyBigInteger removeZero(MyBigInteger myBigInteger){
		//Remove excess zeros
		if(myBigInteger.coefficients.size() == 0 || myBigInteger.coefficients.size() == 1)
			return myBigInteger;

		while(myBigInteger.coefficients.get(myBigInteger.coefficients.size()-1) == 0){
			myBigInteger.coefficients.remove(myBigInteger.coefficients.size()-1);
			if(myBigInteger.coefficients.size() == 0)
				break;
			if(myBigInteger.coefficients.size() == 1 && myBigInteger.coefficients.get(0) == 0)
				break;
		}
		return myBigInteger;
	}
